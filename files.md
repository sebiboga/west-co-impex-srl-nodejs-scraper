...
| `company.js` | Validates company via cuifirma.ro + Peviitor APIs, checks if company is active/inactive |
...
| File | Description |
| ------ | ------------- |
| `src/anaf.js` | cuifirma.ro MCP core module - exports getCompanyFromANAF(cif), getCompanyFromANAFWithFallback(cif, cached), searchCompany(brandName) |
| `src/markdown-generator.js` | Generates docs/jobs.md - exports generateJobsMarkdown(companyData, jobs) |
| `src/job-validator.js` | Shared validation primitives - exports validateByHead(url), validateByContent(url, opts), DEFAULT_EXPIRED_KEYWORDS. Used by both `validate-jobs.js` and `tests/validate-company-jobs.js`. |
...
| `tests/validate-company-jobs.js` | **Company-specific fast validator (used by CI).** HEAD requests only. Called nightly by `automation-testing.yml`. Supports `--dry-run` and `--delete`. |
| `tests/unit/index.test.js` | Unit tests for index.js - parseJobs, mapToJobModel, transformJobsForSOLR |
...
| `tests/unit/demoanaf.test.js` | Unit tests for cuifirma.ro search and company retrieval with mocked responses |
| `tests/integration/workflow.test.js` | Integration tests - cuifirma.ro live API, Peviitor API, SOLR company/job cores |
| `tests/e2e/scraper.test.js` | E2E tests - full pipeline with real careers page, cuifirma.ro, and SOLR |
...
| `ROBOTS.md` | robots.txt analysis and scraping policy for West Company |
...
| File | Description |
| ------ | ------------- |
| `company.json` | **cuifirma.ro cache (committed).** Survives between CI runs so the scraper does not hit cuifirma.ro on every scrape. Refreshed when older than 7 days (configurable via `CACHE_MAX_AGE_DAYS` in company.js). |
...
| `delete_request.json` | **Manual maintenance tool** — SOLR payload to delete ALL jobs for CIF 4565806. Use only when you need to wipe West Company jobs from SOLR entirely. Run with: `curl --user "${SOLR_AUTH}" "https://solr.peviitor.ro/solr/job/update?commit=true" -H "Content-Type: application/json" -d @delete_request.json` |
...
- Full workflow: validate company (cuifirma.ro+Peviitor) → scrape West Company → transform → upsert SOLR → generate docs/jobs.md
