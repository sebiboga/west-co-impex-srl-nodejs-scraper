...
All scraper code, CI workflows, and the static HTML read from this file. You should not need to edit constants in `index.js`, `company.js`, `demoanaf.js`, `tests/validate-company-jobs.js`, `docs/index.html`, or `.github/workflows/automation-testing.yml`.
...
| `tests/validate-company-jobs.js` | Rename to `validate-<brand>-jobs.js` |

**Critical — update test files to match the new scraper's API:**

When you replace the scraper's parser function in `index.js` (e.g. `parseApiJobs()`) with a new parser (e.g. `parseHtmlJobs()` for HTML/cheerio), you **must** update `tests/unit/index.test.js` accordingly:
...
- Each careers site uses a different API/HTML structure — rewrite `fetchJobsPage()` and `parseJobs()` in `index.js` to match the new source
...
```bash
# Clone your fork
git clone https://github.com/sebiboga/west-co-impex-srl-nodejs-scraper.git
...
Open a [GitHub Issue](https://github.com/sebiboga/west-co-impex-srl-nodejs-scraper/issues) with:
